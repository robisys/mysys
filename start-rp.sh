...
--------------------
 install-docker-on-windows-behind-a-proxy
docker-machine ssh default

 sudo -s
Then configure the proxy:

echo "HTTP_PROXY=http://internet.magdeburg.de:8000" >> /var/lib/boot2docker/profile
echo "HTTPS_PROXY=http://internet.magdeburg.de:8000" >> /var/lib/boot2docker/profile
...
from: http://www.netinstructions.com/how-to-install-docker-on-windows-behind-a-proxy/
--------------------
...
echo -e "${BLUE}docker${NC} is configured to use HTTP_PROXY ${GREEN}${HTTP_PROXY} ${NC}  HTTPS_PROXY ${GREEN}  ${HTTPS_PROXY} ${NC}"
date
cd
